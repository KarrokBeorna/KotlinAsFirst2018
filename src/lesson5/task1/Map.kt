...
fun canBuildFrom(chars: List<Char>, word: String): Boolean {
    val a = chars.map { it.toLowerCase() }
    return word.toLowerCase().all { it in a }
}
...
fun hasAnagrams(words: List<String>): Boolean {
    ...
    for (i in words)
        if (i == "") count++
    val wordsAnswer = words.filter { it != "" }
    if (count >= 2)
        for (i in 0 until wordsAnswer.size) {
            val list = mutableListOf<Char>()
            for (element in wordsAnswer[i])
                list.add(element)
            for (k in (i + 1) until wordsAnswer.size) {
                for (its in wordsAnswer[k])
                    if (its in list) answer.add(its)
                if (answer.sorted() == list.sorted()) break else answer.clear()
            }
            if (answer.isNotEmpty()) break
        }
    return (answer.isNotEmpty() || count >= 2)
}
...
fun bagPacking(treasures: Map<String, Pair<Int, Int>>, capacity: Int): Set<String> {
    ...
    var setAnswer = mutableSetOf<String>()
    var sumAnswer = 0
    for ((treasure, intelligence) in treasures)
        if (intelligence.first < capacity) map += treasure to intelligence
    for ((treasure, intelligence) in map) {
        val set = mutableSetOf<String>()
        var sum = intelligence.second
        var weight = capacity - intelligence.first
        set.add(treasure)
        if (sum > sumAnswer) {
            setAnswer = set
            sumAnswer = sum
        ...
        for ((t, i) in map) {
            if (t != treasure && weight > 0 && i.first <= weight) {
                weight -= i.first
                sum += i.second
            }
            if (sum > sumAnswer) {
